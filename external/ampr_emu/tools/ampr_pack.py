    ...
    def __call__(self, progress: BuildProgress) -> None:
        ...
        if progress.phase == "packing":
            ...
            if packing_seconds > 0 and progress.logical_bytes_done > 0:
                ...
                details += f", {_progress_size(int(rate))}/s"
...
VerificationProgressCallback = Callable[[int, int, int, int, str], None]


class ConsoleVerificationProgress:
    """Throttled byte progress for pack verification and source comparison."""
    ...
    def __call__(
        ...
        complete = done >= total
        if complete and self.last_percent == 100:
            return
        if self.last_output and not complete:
            since_output = now - self.last_output
            if since_output < self.min_interval:
                return
            if percent <= self.last_percent and since_output < self.heartbeat_interval:
        ...
        details = (
            ...
            f"{_progress_size(done)}/{_progress_size(total)}"
        )
        if elapsed_seconds > 0 and done > 0:
            rate = done / elapsed_seconds
            details += f", {_progress_size(int(rate))}/s"
        if current_path:
            details += f", current={current_path}"
        print(
            f"[{self.stage} {percent:3d}%] {self.label}: {details}, elapsed {elapsed_text}",
            file=self.stream,
            flush=True,
        )
        self.last_output = now
...
def verify_packs(
    ...
    progress: VerificationProgressCallback | None = None,
) -> dict[str, int]:
    ...
    processed_chunks = 0
    processed_raw = 0
    progress_files_total = sum(
        1
        for file_id in ids
        if manifest.files[file_id - 1].flags & FILE_FLAG_PACKED
    )
    progress_total = sum(
        manifest.files[file_id - 1].logical_size
        for file_id in ids
        if manifest.files[file_id - 1].flags & FILE_FLAG_PACKED
    )
    if progress is not None:
        progress(0, progress_total, 0, progress_files_total, "")
        ...
        for file_id in ids:
            ...
            for local in range(record.chunk_count):
                ...
                if prior is not None:
                    ...
                    processed_raw += chunk.raw_size
                    processed_chunks += 1
                    if progress is not None and processed_chunks % 1024 == 0:
                        progress(
                            processed_raw,
                            progress_total,
                            verified_files,
        ...
        for file_id in ids:
            ...
            with source_path.open("rb") as source:
                for local, packed_raw in enumerate(reader.iter_file_data(file_id)):
                    ...
                    compared_chunks += 1
                    if progress is not None and compared_chunks % 1024 == 0:
                        progress(
                            compared_bytes,
                            progress_total,
                            compared_files,
...
def create_parser() -> argparse.ArgumentParser:
    ...
    verify.add_argument(
        "--no-progress",
        action="store_true",
        help="suppress verification progress on stderr; final JSON still goes to stdout",
    )
    ...
    try:
        if args.command == "pack":
            ...
            result = verify_packs(
                args.index,
                args.file,
                progress=None if args.no_progress else ConsoleVerificationProgress(
                    "verify", "verifying packed chunks"
                ),
            )
            if args.root is not None:
                result["source_compare"] = verify_packs_against_root(
                    ...
                    args.file,
                    progress=None if args.no_progress else ConsoleVerificationProgress(
                        "compare", "comparing against source"
                    ),
                )
            _print_json(result)
